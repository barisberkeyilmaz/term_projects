...
void addTeam(string team_name)
{
    ...
    Team *temp_tarr = new Team[current_size];

    for(int i = 0; i < current_size - 1; i++){
        temp_tarr[i].setName(tarr[i].getName());
        temp_tarr[i].setId(tarr[i].getId());
        for (int j = 0; j < tarr[i].getCurrentSize(); j++){
            temp_tarr[i].addPlayer(tarr[i].getPlayerList()[j].getNumber(),
            tarr[i].getPlayerList()[j].getName(),
            tarr[i].getPlayerList()[j].getAge(),
            tarr[i].getPlayerList()[j].getGoalNumber(),
            tarr[i].getPlayerList()[j].getAssistNumber(), 
            tarr[i].getPlayerList()[j].getMatchNumber());
        }
    ...
    temp_tarr[current_size-1].setName(team_name);
    temp_tarr[current_size-1].setId(current_size - 1);
...
void removeTeam(int teamId)
{
    ...
    for(int i = 0; i < teamId; i++){
        temp_tarr[i].setName(tarr[i].getName());
        temp_tarr[i].setId(tarr[i].getId()-1);
        for (int j = 0; j < tarr[i].getCurrentSize(); j++){
            temp_tarr[i].addPlayer(tarr[i].getPlayerList()[j].getNumber(),
            tarr[i].getPlayerList()[j].getName(),
            tarr[i].getPlayerList()[j].getAge(),
    ...
    for(int i = teamId + 1 ; i < current_size + 1; i++){
        temp_tarr[i-1].setName(tarr[i].getName());
        temp_tarr[i-1].setId(tarr[i].getId()-1);
        for (int j = 0; j < tarr[i].getCurrentSize(); j++){
            temp_tarr[i-1].addPlayer(tarr[i].getPlayerList()[j].getNumber(),
            tarr[i].getPlayerList()[j].getName(),
            tarr[i].getPlayerList()[j].getAge(),
            tarr[i].getPlayerList()[j].getGoalNumber(),
            tarr[i].getPlayerList()[j].getAssistNumber(), 
